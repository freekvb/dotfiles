...
## youtube search, play and control with dmenu

## To use this you will need socat installed
socket_path="/tmp/mpvsocket"
list_file="$HOME/.config/pipecat_turbo_lists"
...
add_to_list(){
	file=$( cat $list_file )
	start_index=$( cat "$HOME""/.config/pipecat_turbo_lists" | grep -n "####- START LIST <$1>" | awk -F\: '{ print $1 }' )
	file_lenght=$( cat  "$HOME""/.config/pipecat_turbo_lists" | wc -l )
	if [[ -z $start_index ]]
		then
		echo "doesn't exist"
		break
	...
	echo "$( cat "$HOME""/.config/pipecat_turbo_lists" | sed "$index"'i'"$2" )" > $list_file
}
...
do
	...
	search_option=$( echo -e "Controlls\nSearch\nAudio mode\nShuffle mode\nYour Lists\nList tools" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Option: " )
	...
	do
		...
		then
			search_option=$( echo -e "Controlls\nSearch\nAudio mode\nShuffle mode\nYour Lists\nList tools" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Audio mode set: " )
			audio_mode="--no-video"
		fi
		...
		then
			search_option=$( echo -e "Controlls\nSearch\nAudio mode\nShuffle mode\nYour Lists\nList tools" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Shuffle mode set: " )
	...
	then
		search_option=$( echo -e "Add to list\nCreate new list" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "List tools:" )
	fi

	## If you selected add to list, relaunch the menu in add to list mode
	if [[ $search_option == "Add to list" ]]
	then
		list=$(cat "$HOME""/.config/pipecat_turbo_lists" | grep "####- START
        LIST" | awk -F\< '{ print $2 }' | awk -F\> '{ print $1 }' | dmenu -sb '#777a84' -fn "Terminus:size:10" -l 20 )
		search_option=$( echo -e "Video\nPlaylist\nChannel" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Find what you want to add: " )
		add_to_list="True"
	fi
	...
	then
		search_option=$( echo -e "Video\nPlaylist\nChannel" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Search on youtube: " )
	fi
	...
	then
		...
		video=$( echo "" | dmenu -sb '#777a84' -l 0 -fn "Terminus:size:10" -p "Search for video:"  )
		...
		choice=$( echo "$titles" | dmenu -sb '#777a84' -fn "Terminus:size:10" -l 20 )
	...
	then
		...
		playlist=$( echo "" | dmenu -sb '#777a84' -l 0 -fn "Terminus:size:10" -p "Search for a playlist:"  )
		...
		choice=$( echo "$titles" | dmenu -sb '#777a84' -fn "Terminus:size:10" -l 20 -i )
	...
	then
		...
		channel=$( echo "" | dmenu -sb '#777a84' -l 0 -fn "Terminus:size:10" -p "Search for a channel:"  )
	...
	then
		...
		choice=$( echo -e "||\nUp\nDown\n<<\n>>\nX" | dmenu -sb '#777a84' -l 0 -fn "Terminus:size:10" -p "$currently_playing" )
		case $choice in
			"<<")
	...
	then
		...
		list=$(cat "$HOME""/.config/pipecat_turbo_lists" | grep "####- START
        LIST" | awk -F\< '{ print $2 }' | awk -F\> '{ print $1 }' | dmenu -sb '#777a84' -fn "Terminus:size:10" -l 20 -i )
		...
		echo $list
		start_index=$( cat "$HOME""/.config/pipecat_turbo_lists" | grep -n "####- START LIST <$list>" | awk -F\: '{ print $1 }' )
		file_lenght=$( cat  "$HOME""/.config/pipecat_turbo_lists" | wc -l )
		...
		play_index=$( echo "$titles" | grep -n "" | dmenu -sb '#777a84' -fn "Terminus:size:10" -l 20 -i | awk -F\: '{print $1}' )
	...
	then
		list_name=$( echo "" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Enter your list name" )
		...
		lists=$(cat "$HOME""/.config/pipecat_turbo_lists" | grep "####- START LIST <""$list_name""> -####" )
		if [[ -v lists ]]
		then
			echo "Ok" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Playlist already exists"
			break
		fi
		...
		then
			echo "Ok" | dmenu -sb '#777a84' -fn "Terminus:size:10" -p "Playlist name can't use < or > or \\n"
			break
		fi
